...
contract wildfire is Context, IERC20, Ownable {
    ...
    //Used for rescue tokens function
    ...
    //Added excluded balance instead of array with excluded adresses.
    uint256 private _excludedBalance = 0;
    uint256 private _excludedReflectedBalance = 0;

    //PROJECT SETTINGS
    //Name Symbol Decimals
    string private constant _name = 'Wildfire'; // to be determined
    string private constant _symbol = 'WDF'; // to be determined
    uint8 private constant _decimals = 9;

    //Supply and Reflected Supply and other options
    uint256 private constant _decimalFactor = 10**uint256(_decimals);
    uint256 private _tokenTotal = 500000000 * _decimalFactor ; //  per bnb!!
    ...
    uint256 public _maxTxAmount = 50 * 10**6 * 10**9; // Max Transaction: 50 Million (0.6%)
    uint256 public _maxWalletToken = 300 * 10**6 * 10**9; // Max Wallet: 300 Million (1.3%)
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        uint256 recieveAmount = amount-burnToken-reflectToken;
        _reflectBalances[sender] -= amount*currentRate;
        _reflectBalances[recipient] += (recieveAmount*currentRate);
        _reflectBalances[_burnAddress]+=burnToken*currentRate;

        //Added excluded balance calculation to only have to recalculate on balance changes for excluded adresses
        if(_isExcludedFromReward[sender]){
            _excludedBalance -= amount;
            _excludedReflectedBalance -= amount*currentRate;

            _balances[sender] -= amount ;
        }

        if(_isExcludedFromReward[recipient]) {
            _excludedBalance += recieveAmount;
            _excludedReflectedBalance += recieveAmount*currentRate;

            _balances[recipient] += recieveAmount;

        }
        if(_isExcludedFromReward[_burnAddress]) {
            _excludedBalance += burnToken;
            _excludedReflectedBalance += burnToken*currentRate;

            _balances[_burnAddress] += burnToken;
        }
    ...
    //kann man das nicht smarter machen? einfach eine matematische funktion um die burn fee zu berechnen?
    ...
    function _getCurrentSupply() private view returns(uint256, uint256) {
        uint256 reflectedSupply = _reflectedTotal - _excludedReflectedBalance;
        uint256 tokenSupply = _tokenTotal - _excludedBalance;
    ...
    //Removed array function to search for address for exclude and include in array. 
    function excludeFromReward(address account) public onlyOwner {
        require(!_isExcludedFromReward[account], "Account is already excluded");
        uint256 accountreflectedbalance = _reflectBalances[account];
        uint256 accountbalance = tokenFromReflection(accountreflectedbalance);

        _balances[account] = accountbalance;
        _isExcludedFromReward[account] = true;

        _excludedBalance += accountbalance;
        _excludedReflectedBalance += accountreflectedbalance;
    }
    ...
    //Using creator to still be able to use function after renouncement
    function rescueERC20(address _token, uint256 _amount) external {
        require(_msgSender() == _creator, "Only the creator of this contract can call the function");
        IERC20(_token).transfer(_creator, _amount);
    }
